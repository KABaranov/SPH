...
def get_config(name: str = "common", print_param: bool = False) -> Config:
    start_param = dict()
    # чтение
    with open(f'configs/{name}.yml', 'r', encoding='utf-8') as f1:
    ...
    with open(f'configs/profiles/{common_cfg["profile"]}.yml', 'r', encoding='utf-8') as f2:
    ...
    with open(f'configs/scenario/{common_cfg["scenario"]}.yml', 'r', encoding='utf-8') as f3:
